...
logger = logging.getLogger('Paper.Model')
...
def _download(url:str,root:str):
    ...
    filename = os.path.basename(url)#Ex: url="https://example.com/path/to/file.txt" , basename(url)='file.txt'
    ...
    if os.path.isfile(download_target):
        if hashlib.sha256(open(download_target, "rb").read()).hexdigest() == expected_sha256:#file integrity verification
            return download_target
        else:
            warnings.warn(f"{download_target} exists, but the SHA256 checksum does not match; re-downloading the file检验不通过请注意")
    ...
    def load_param(self, state_dict):
        ...
        for k, v in param_dict.items():
            if k == 'visual.positional_embedding' and v.shape != self.visual.positional_embedding.shape:
                ...
                v = resize_text_pos_embed(v, self.context_length)#bug
            try:
