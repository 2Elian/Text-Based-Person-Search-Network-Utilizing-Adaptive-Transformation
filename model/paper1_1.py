...
class ADT_NET(nn.Module):
    def __init__(self, args, num_classes=11003,num_fpn_levels=2):
        super().__init__()
        self.args = args
    ...
    def forward(self, batch , cur_epoch=1):
        ...
        if 'dma' in self.current_task:
            ret.update({'dma_loss':objectives.bi_sdm(i_feats, t_feats, batch['pids'], logit_scale,self.args.num_epoch,cur_epoch)})

        if 'sdm' in self.current_task:
            ret.update({'sdm_loss':objectives.compute_sdm(i_feats, t_feats, batch['pids'], logit_scale)})
            
        if 'asdm' in self.current_task:
            ret.update({'asdm_loss':objectives.compute_asdm(i_feats, t_feats, batch['pids'], logit_scale, 0.5, 0.035)})
        ...
        if 'style' in self.current_task:
            ...
            style_v = torch.mean(style_v,dim=1)#[b,512]
            ...
            for i, m in enumerate(fpns):
                outputs.append(
                    m(hat_v.float()))
            hat_v_1 = outputs[0]#torch.Size([64, 512, 48, 16])
            hp1,wp1 = hat_v_1.shape[2] , hat_v_1.shape[3]
            hat_v_2 = outputs[1]#torch.Size([64, 512, 24, 8]) 拿它做mi
            hat_v_1 = torch.transpose(torch.reshape(hat_v_1,[b,d,hp1*wp1]),2,1)#[b,48*16,512]
            hat_v_2 = torch.transpose(torch.reshape(hat_v_2,[b,d,hp*wp]),2,1)#[b,24*8,512]
            hat1 = torch.mean(hat_v_1,dim=1)
            hat2 = torch.mean(hat_v_2,dim=1)
            ret.update({'style_loss': objectives.compute_style1_1(i_feats,hat1,hat2,style_v,logit_scale)})
...
def build_model(args, num_classes=11003):
    model = ADT_NET(args, num_classes,num_fpn_levels=3)
